...
B4 六类负载及双后端小规模端到端测试
 ↓
B5 历史模板能力、读取预热、计时外全量正确性验证
 ↓
B6 逐RunUnit Load→Run、CLI、240次矩阵编排、CSV、汇总和Markdown/SVG报告工具
 ↓
...
| B4 | 六类负载 | 已验收 | `0a771808f3b7ccf096eb02ca4b90734b8b22bc3f` |
| B5 | 模板恢复与正确性验证（历史能力保留） | 已验收 | `27831c17074b7bc4f4cd8f3727540ea647d908af` |
| B6 | 逐RunUnit Load→Run、矩阵编排与报告工具 | 已验收 | `5d0c3958408bbb99384e6e8ac8ace3da5afd790a` |
| B7 | Mac正式跑测与报告 | 已验收 | —（正式跑测与结果生成移交用户） |

B4 独立 Review 后经用户批准，将 `benchmarks/src/trace.rs` 纳入 B4 文件范围，用于保存并校验 Trace 的完整生成配置来源。

B6 独立 Review 后曾经用户批准加入的跨进程密封模板方案，在 B6 验收和提交前由用户明确废弃；相关未验收实现必须移除。B6 改为逐 RunUnit 直接 Load→Run，CLI 删除 `prepare`，B6/B7 禁止调用任何模板恢复、密封、COW 克隆或物理复制路径。`benchmarks/src/fs.rs`、`benchmarks/src/template.rs` 及对应测试只允许用于移除本轮尚未验收的密封模板扩展；B5 已提交的历史模板能力本身保留。

B6 验收提交后增加的 `custom-run` 和 `scripts/run_custom.sh` 只用于用户指定单个 Backend、负载、线程数和条目数的非正式快速测试。它复用B6逐RunUnit生命周期，但使用独立结果格式，禁止进入B7正式矩阵或正式报告。

B7 的 Benchmark 实现、执行入口和文档规范已经由用户验收；当前 Mac 上的实际性能跑测、结果汇总和报告生成由用户自行执行。因此本表的“已验收”表示实现交付与执行责任完成移交，不表示仓库中已经存在 240 行正式结果、六张图或最终性能报告；这些产物只有在用户按 B7 文档实际完成正式矩阵后才成立。
