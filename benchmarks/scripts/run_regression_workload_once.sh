...
usage() {
    cat <<'EOF'
Usage:
  run_regression_workload_once.sh [--output-root ABS_PATH] [--dry-run]
...
The matrix is fixed to:
  ...
  repetitions: 1 per valid combination
...
The script executes 186 RunUnits under 186 combination directories and reports
six unsupported combinations (six omitted repetitions).

The default output root is $HOME/work/result. Each successful existing
repetition is skipped; an incomplete or mismatched combination/repetition
stops the run. Use a new output root when comparing a different RustKV commit.
EOF
}
...
run_combination() {
    ...
    if [ "$REQUESTS" -lt "$THREADS" ]; then
        ...
        printf '[skip unsupported %d/6] backend=%s workload=%s records=%s threads=%s: only %s requests; omitted repetitions=1\n' \
...
if [ "$DRY_RUN" = true ]; then
    echo "dry-run listed 186 executable RunUnits in 186 combinations and 6 unsupported combinations (6 omitted repetitions): $OUTPUT_ROOT"
else
    echo "all 186 executable custom benchmarks completed in 186 combinations; skipped combinations=6; omitted repetitions=6: $OUTPUT_ROOT"
fi
